...
struct Item {
  ...
  void append(const Item& other) {
    ...
    ABORT_IF(other.shape[-1] != other.shape.elements(),
              "2 - Only flat items can be appended: {}",
              other.shape);

    // cut to size (get rid of padding if any) to make append operation work correctly
    size_t bytesWithoutPadding
        = shape.elements() * sizeOf(type);
    bytes.resize(bytesWithoutPadding);

    shape.set(-1, shape.elements() + other.shape.elements());

    size_t addbytesWithoutPadding
        = other.shape.elements() * sizeOf(other.type);  // ignore padding if any
    ...
    size_t multiplier
        = (size_t)ceil((float)bytes.size() / (float)256);
  ...
  void convertFromTo() {
    ...
    for(int i = 0; i < elements; ++i) + out[i] = (To)in[i];
