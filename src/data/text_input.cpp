    ...
    for(size_t i = 0; i < files_.size(); ++i) {
      ...
      if(io::getline(dummyStream, line)) {
        ...
        if(options_->get<bool>("xml-input", false)) {
          ABORT_IF(!targetVocab_, "The target vocabulary is not set for processing XML input");
          std::string strippedLine;
          xml::processXml(line, strippedLine, targetVocab_, tup);
          line = strippedLine;
        }
        words = vocabs_[i]->encode(line, /*addEOS =*/true, inference_);
