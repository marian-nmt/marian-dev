...
void CorpusBase::processXml(const std::string& line,
  ...
  for (size_t xmlTokenPos=0 ; xmlTokenPos < xmlTokens.size() ; xmlTokenPos++) {
    ...
    if (!isXmlTag(xmlTokens[xmlTokenPos])) {
      ...
      Split(stripped_line, outputWords, " "); // ???
    ...
    // xml tag
      ...
      if (isClosed && isUnary) {
        std::cerr << "XML ERROR: can't have both closed and unary tag <" << tag << ">: " << line << std::endl;
        continue;
      }
      ...
      if (isClosed || isUnary) {
        ...
        if (tagStack.size() == 0) {
          std::cerr << "XML ERROR: tag " << tagName << " closed, but not opened" << ":" << line << std::endl;
          continue;
        }
        ...
        if (openedTag.first != tagName) {
          std::cerr << "XML ERROR: tag " << openedTag.first << " closed by tag " << tagName << ": " << line << std::endl;
          continue;
        }
        ...
        if (! span.empty()) {
          ...
          if (ij.size() != 1 && ij.size() != 2) {
            std::cerr << "XML ERROR: span attribute must be of the form \"i-j\" or \"i\": " << line << std::endl;
          }
          else {
