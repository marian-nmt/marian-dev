...
public:
  CSR_GPU(int rows, int cols, size_t device)
      : CSR(rows, cols, device, DEVICE_GPU) {
  ...
  CSR_GPU(int rows,
      ...
      size_t device)
      : CSR(rows, cols, values, rowIndices, colIndices, device, DEVICE_GPU) {
    cudaSetDevice(device);
    CUSPARSE_CHECK(cusparseCreate(&handle_));
    CUSPARSE_CHECK(cusparseCreateMatDescr(&descr_));
    CUSPARSE_CHECK(cusparseSetMatType(descr_, CUSPARSE_MATRIX_TYPE_GENERAL));
  ...
  CSR_GPU(Tensor dense) : CSR(dense, DEVICE_GPU) {
  ...
  std::string debug() {
    ...
    Tensor tensor(new TensorGPU(mem, {rows(), cols()}, device_));
    toTensor(tensor);
...
void multiply(
    Ptr<CSR_GPU>, const Ptr<CSR_GPU>, const Ptr<CSR_GPU>, bool = false, bool = false);

void LfaForward(Tensor out, Tensor logits, Tensor att, Ptr<CSR_GPU> sparseLf);

void LfaBackward(Tensor grad, Tensor adj, Ptr<CSR_GPU> sparseLf);
