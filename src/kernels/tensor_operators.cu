...
void ConcatN(Ptr<Allocator<DeviceGPU>> allocator,
  ...
  mp_ins->insert(h_ins, num);

  auto mp_lengths = allocator->alloc<int>(num);
  mp_lengths->insert(h_lengths, num);
...
__global__ void gMaxPoolingForward(
    ...
    float* mask, int numKernels, int maskCols,
    int width, int lastWidth)
{
  ...
  // printf("tid: %d\trowId: %d\tcolId: %d\tb: %d\tout:%d\n", tid, rowId, colId, (rowId * inCols) + (colId * width), rowId + (colId * outCols));
...
void MaxPoolingForward(Tensor out, Tensor in, Tensor mask, int width, bool isEven) {
  ...
  int lastWidth = ((inCols - isEven) % width == 0) ? width : (inCols - isEven) % width;
...
__global__ void gMaxPoolingBackward(
    float* adj, int adjRows, int adjCols,
    float* in, float* adjIn, int inRows, int inCols,
    float* mask, int numKernels, int maskCols,
    int width, int lastWidth)
{
  int tid = threadIdx.x + blockIdx.x * blockDim.x;
...
void MaxPoolingBackward(Tensor adj, Tensor adjIn, Tensor in, Tensor mask, int width, bool isEven) {
...
void Concatenate0(Tensor out, const std::vector<Tensor>& inputs) {
  ...
  for(auto in : inputs) {
    ...
    cudaMemcpyAsync(out->data() + offset,
                    in->data(),
