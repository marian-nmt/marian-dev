...
// shared base class for transformer-based EncoderTransformerInt and DecoderTransformerInt
...
class TransformerInt : public EncoderOrDecoderBase {
...
public:
  TransformerInt(Ptr<Options> options)
  ...
  // Expr postProcess_int(std::string prefix, std::string ops, Expr input, Expr prevInput, Expr unquant, float dropProb = 0.0f) const {
  //   auto output = input;

  //   for(auto op : ops) {
  //     // dropout
  //     if(op == 'd') {
  //       output = cpu::int8::unquantize(output, unquant);
  //       output = dropout(output, dropProb);
  //       // output = cpu::int8::quantize(output, 1.0 / unquant);
  //     }
  //     // skip connection
  //     else if(op == 'a') {
  //       // output = cpu::int8::unquantize(output, unquant);
  //       output = output + prevInput;
  //       // prevInput->debug("prevInput");
  //       // auto xxx = cpu::int8::quantize(prevInput, 1.0 / unquant);
  //       // xxx->debug("xxx");
  //       // output = cpu::int8::elemwiseAdd(output, xxx);
  //       // output = cpu::int8::quantize(output, 1.0 / unquant);
  //     }
  //     // highway connection
  //     else if(op == 'h') {
  ...
  Expr DecoderLayerRNN(rnn::State& decoderState,
    ...
    auto unquant = rnn->lastCellStates()[0].unquant;

    decoderState = {
      cpu::int8::unquantize(rnn->lastCellStates()[0].output, unquant),
      cpu::int8::unquantize(rnn->lastCellStates()[0].cell, unquant),
    };
    output = transposeTimeBatch(output);
...
class EncoderTransformerInt : public TransformerInt<EncoderBase> {
public:
  EncoderTransformerInt(Ptr<Options> options) : TransformerInt(options) {}
  virtual ~EncoderTransformerInt() {}
...
class TransformerIntState : public DecoderState {
public:
  TransformerIntState(const rnn::States& states,
                   Expr logProbs,
  ...
  virtual Ptr<DecoderState> select(const std::vector<IndexType>& selIdx,
    ...
    auto selectedState = New<TransformerIntState>(states_.select(selIdx, beamSize, /*isBatchMajor=*/true), logProbs_, encStates_, batch_);
...
class DecoderTransformerInt : public TransformerInt<DecoderBase> {
private:
...
public:
  DecoderTransformerInt(Ptr<Options> options) : TransformerInt(options) {}
  ...
  virtual Ptr<DecoderState> startState(
    ...
    if (layerType == "rnn") {
      ...
      // don't use TransformerIntState for RNN layers
      return New<DecoderState>(startStates, nullptr, encStates, batch);
    }
    else {
      rnn::States startStates;
      return New<TransformerIntState>(startStates, nullptr, encStates, batch);
    }
  }
  ...
  Ptr<DecoderState> step(Ptr<DecoderState> state) {
    ...
    if (opt<std::string>("transformer-decoder-autoreg", "self-attention") == "rnn") {
      ...
      nextState = New<TransformerIntState>(
          decoderStates, logits, state->getEncoderStates(), state->getBatch());
    }
...
// factory functions
Ptr<EncoderBase> NewEncoderTransformerInt(Ptr<Options> options)
{
  return New<EncoderTransformerInt>(options);
}

Ptr<DecoderBase> NewDecoderTransformerInt(Ptr<Options> options)
{
  return New<DecoderTransformerInt>(options);
}
