...
public:
  Tanh(Ptr<ExpressionGraph> graph, Ptr<Options> options) : Cell(options) {
    ...
    if(layerNorm_) {
      if(dimInput)
        gamma1_ = graph->param(
            prefix + "_gamma1", {1, 3 * dimState}, inits::ones());
      gamma2_ = graph->param(
          prefix + "_gamma2", {1, 3 * dimState}, inits::ones());
    }
  }
...
public:
  FastLSTM(Ptr<ExpressionGraph> graph, Ptr<Options> options) : Cell(options) {
    ...
    if(layerNorm_) {
      if(dimInput)
        gamma1_ = graph->param(
            prefix + "_gamma1", {1, 4 * dimState}, inits::ones());
      gamma2_ = graph->param(
          prefix + "_gamma2", {1, 4 * dimState}, inits::ones());
    }
  }
  ...
  Multiplicative(Ptr<ExpressionGraph> graph, Ptr<Options> options)
    ...
    if(CellType::layerNorm_) {
      gamma1m_ = graph->param(
          prefix + "_gamma1m", {1, dimState}, inits::ones());
      gamma2m_ = graph->param(
          prefix + "_gamma2m", {1, dimState}, inits::ones());
    }
  }
